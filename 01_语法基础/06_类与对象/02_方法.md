...
- `@classmethod` 是一个装饰器，用于将方法声明为“类方法”。
- 类方法的第一个参数是 `cls`，代表类本身，而不是实例（实例方法的第一个参数是 `self`）。
- 类方法可以通过类名或实例来调用，但它操作的是类本身的数据（如类属性），而不是某个具体实例的数据。

### 2. 典型用途

- **工厂方法**：实现多种不同的实例化方式。
- **访问/修改类属性**：在方法中访问或修改类属性时，使用类方法更合适。
- **继承友好**：在继承体系中，`cls` 会自动指向当前子类，而不是父类。
...
- 普通方法（实例方法）第一个参数是 `self`，只能操作实例属性。
- 类方法第一个参数是 `cls`，可以操作类属性。
- 普通方法不能直接访问类本身，类方法可以。
- 类方法支持继承，`cls` 会自动指向子类。
...
Sub.show_self(Sub())  # 输出 1
Sub.show_cls()        # 输出 2
```

### 5. 总结

- `@classmethod` 让你可以写出更灵活、可扩展的类和工厂方法。
- 选择普通方法还是类方法，取决于你希望方法操作“实例”还是“类”。
